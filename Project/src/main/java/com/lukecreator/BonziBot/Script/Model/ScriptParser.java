...
public class ScriptParser {
	...
	public static ScriptStatementCollection parseCode(Script parent, String... lines) throws ScriptParseException {
		ScriptStatementCollection ssc = new ScriptStatementCollection(parent);
		for(String line: lines)
			ssc.add(parseLine(line));
		return ssc;
	}
	public static ScriptStatement parseLine(String line) throws ScriptParseException {
		...
		for(StatementDescriptor sd: EditorCategories.getAllPossibleStatements()) {
			...
			if(!keyword.equalsIgnoreCase(sdKeyword))
				continue;
			try {
				statement = sd.internalClass.newInstance();
			} catch (InstantiationException e) {
				e.printStackTrace();
				return null;
				//throw new ScriptParseException("Internal error. '" + e.getMessage() + "'");
