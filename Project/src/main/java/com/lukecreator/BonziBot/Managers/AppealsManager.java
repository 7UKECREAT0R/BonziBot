...
public class AppealsManager implements IStorableData {
	...
	public void processButtonEvent(String[] parts, BonziBot bb, ButtonInteractionEvent event) {
		if(parts[1].equals("request")) {
			...
			bb.eventWaiter.waitForResponse(uid, appealMessage -> {
				...
				MessageChannelUnion channel = appealMessage.getChannel();
		...
		if(parts[1].equals("accept")) {
			...
			String tag = mod.getUser().getEffectiveName();
			...
			guild.unban(UserSnowflake.fromId(bannedUser)).reason
				("Appeal Accepted by Moderator " + mod.getUser().getName()).queue();
			...
			event.getHook().editOriginalComponents(new ArrayList<LayoutComponent>()).queue();
	...
	public boolean hasMercy(long guild, long user) {
		List<Long> m = this.mercy.get(guild);
		if(m == null)
			return false;
		for(long test: m) {
			if(user == test)
	...
	public void addAppeal(long guild, BanAppeal appeal) {
		List<BanAppeal> existing = this.appeals.get(guild);
		if(existing == null)
			existing = new ArrayList<BanAppeal>();
		
		existing.add(appeal);
		this.appeals.put(guild, existing);
	}
	...
	public BanAppeal getAppeal(long guild, long bannedUser) {
		if(this.appeals.containsKey(guild)) {
			List<BanAppeal> existing = this.appeals.get(guild);
	...
	public void saveData() {
		DataSerializer.writeObject(this.mercy, "mercy");
	}
	...
	public void loadData() throws EOFException {
		...
		this.mercy = (HashMap<Long, List<Long>>)o;
	}
}
