...
public class GuildSettingsManager implements IStorableData {
			...
			if(role != null) {
				try {
					...
					.handle(ErrorResponse.MISSING_PERMISSIONS, err -> {
						if(tc != null)
							tc.sendMessageEmbeds(BonziUtils.failureEmbed("I need some help here!",
					...
					if(exc instanceof HierarchyException) {
						...
						if(tc != null)
							tc.sendMessageEmbeds(BonziUtils.failureEmbed("something went pretty wrong",
	...
	public void simulateMemberJoin(BonziBot bb, Member member) {
	...
	public void simulateMemberLeave(BonziBot bb, Member member) {
		Guild guild = member.getGuild();
		long guildId = guild.getIdLong();
		User user = member.getUser();
		GuildSettings settings = this.getSettings(guildId);
