...
public class ScriptCache {
	...
	public static void registerTimer(long guildId, InvocationTimed invoke, ScriptPackage pkg, Script script) {
		if(DEBUG_MESSAGES)
			InternalLogger.print("Scripting - register timer: " + script.name + ", delay: " + invoke.time.toShortString());
	...
	public static boolean shouldCheckScripts(SlashCommandEvent event) {
		if(!event.isFromGuild())
			return false;
		long eventId = event.getCommandIdLong();
		return commandIds.contains(eventId);
	}
	public static boolean shouldCheckScripts(ButtonClickEvent event) {
	...
	public static void pollTimedScripts(BonziBot bb, JDA jda) {
		...
		for(ScriptTimer timer: timers) {
			...
			for(ScriptPackage pkg: packages) {
				...
				ScriptContextInfo context = new ScriptContextInfo(null, null, null, null,
						null, null, jda, bb, null, null, guild, settings);
		...
		for(Entry<Long, List<ScriptPackage>> entry: entries) {
			...
			if(!existingCommands.containsKey(guildId)) {
				...
				try {
					...
					InternalLogger.printError("Ignoring script command register because guild " + guild.getName() +
						" (" + guild.getIdLong() + ") is not authorized with applications.commands", Severity.WARN);
					continue;
				}
			}
				...
				for(Script script: scripts) {
					...
					if(impl == Implementation.COMMAND) {
						...
						if(existing.stream().anyMatch(cmd -> cmd.getIdLong() == invokeCmd._commandId)) {
							if(DEBUG_MESSAGES)
								InternalLogger.print("Scripting - NOT registering command: " + invokeCmd.commandName + ", id: " + invokeCmd._commandId + "; already exists");
							continue;
						}
